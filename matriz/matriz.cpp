...
Matriz::Matriz(int qLinhas, int qColunas):quantidadeDeLinhas(0),quantidadeDeColunas(0),ptMatriz(0)
{
...
Matriz* Matriz::operator * (Matriz const * const mat)const
{
  if(mat->getQuantidadeDeColunas() != quantidadeDeLinhas)
     throw QString("Nao pode ser executado a multiplicacao entre as matrizes");

  try
  {
    Matriz *matAux = new Matriz(quantidadeDeLinhas,quantidadeDeColunas);
    int valor = 0;
    ...
    for(int i=0; i<quantidadeDeLinhas; i++)
     {
        ...
        {
                 for(int z=0; z<mat->quantidadeDeLinhas; z++)
                 {
                  valor = valor + (this-> getElemento(i,z) * mat->getElemento(z,j));
                 }
                 matAux -> setElemento(valor,i,j);
                 valor = 0;
        }
     }
